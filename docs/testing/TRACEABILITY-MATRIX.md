...
| §F | **Direction B** — test artifact → the construct it covers | Every file of the new suite, and the production construct or infrastructure obligation behind it | 97 rows naming 99 of the 103 change-set paths |
...
| F39 | `.gitignore` | Infrastructure: 23 active patterns covering every artifact the four suites produce, the readings the dashboard producer writes, the browser evidence a verification run captures, plus the two dependency trees and the documented virtual environments - and nothing else. Ten rules are path-anchored (`backend/reports/`, `frontend/coverage/`, `frontend/reports/`, `docs/testing/reports/`, `e2e/node_modules/`, `e2e/test-results/`, `e2e/playwright-report/`, `e2e/reports/`, `blitzy/screenshots/`, `blitzy/screen_recordings/`) and reach no further than the directory that produces them; the remaining thirteen (`__pycache__/`, `*.py[cod]`, `.pytest_cache/`, `.coverage`, `coverage.xml`, `coverage.json`, `coverage.lcov`, `htmlcov/`, `node_modules/`, `.venv-backend/`, `venv/`, `env/`, `ENV/`) match at any depth by design, which is what covers the coverage files pytest writes into `backend/`. `docs/testing/reports/` is the one this checkpoint added, because a documented command wrote a generated reading into the working tree (D287, D323). No `package-lock.json` rule exists, and the file says at length why |
...
| F100 | `docs/testing/SECURITY-GAPS.md` | Rule 1 and Rule 3: the current-state security gap register. Every security exposure still present in the delivered tree, each stated as a fact about the code, with the AAP clause that puts it out of the authorized write surface, what it is worth to an attacker, and what a human must do. 25 exposures in five groups — application code, pinned dependencies, infrastructure and deployment, the documentation that overstated the position, and the executive deck as a served artifact, the one group fixed by a user-specified rule rather than an AAP exclusion. Covers no production symbol and asserts nothing: it exists because a decline recorded only in a review reply is indistinguishable, later, from a finding nobody noticed (D324) |
...
| G4 | Chart construction in `src/components/Analytics` | The tree-shakeable `{ Chart }` is imported and `Chart.register` is never called, so construction throws in any environment with a canvas | Two assertions at two layers. Under jsdom, the caught-failure path: heading and canvas render, the rejection is logged, the component stays mounted. In a real browser, where a live 2D context carries Chart.js as far as the empty registry, the ceiling **itself**: `e2e/tests/analytics.spec.ts` (F83) opts into the harness stub's forwarding header and asserts the unregistered chart part, the uncaught escape — `renderCharts` has no `try`/`catch` and the effect calling it is passive — and the unmounted route. So this ceiling is no longer stood in for; it is measured |
...
|---|-----------|---------------------------|---------------------|-----------|
| H1 | Every active line of the backend manifest is an exact pin, every pinned version is the version actually installed, **and every third-party module the test tree reaches is provided by a pinned distribution** | `backend/requirements-dev.txt` (25 active lines, all 25 exact pins, `python-jose[cryptography]` among them at `3.5.0`, above the two CVEs the plan's `3.3.0` carries, plus `grpcio` and `grpcio-status`) and `backend/tests/test_dependency_closure.py`, which now closes both directions: manifest ⇄ installed (an exact-pin grammar applied to **every** active line rather than to the lines that happen to match, an assertion that the parsed-pin count equals the active-line count, a distribution-name uniqueness check, and one installed-version assertion per pin) and use ⇒ manifest (an AST import scan, a string-literal scan of `patch` / `import_module` first arguments, and a read of the egress guard's own target tables out of the loaded `tests.conftest`, each resolved to a distribution through `importlib.metadata` and asserted to be pinned) | **72 cases pass**: 25 installed-version assertions against `importlib.metadata.version`, 22 module-provider assertions — one per third-party module path the tree reaches — 12 parametrised non-exact-pin rejection forms, and 13 single cases covering the grammar, the pin/active-line count, distribution uniqueness, comment handling, module-path resolution, the classifier, the two discovery mechanisms, provider specificity and the gRPC target table. Negatively validated twice. Rewriting one `==` as `>=` failed exactly two cases — the exact-pin case naming the line, and the count case reporting `assert 24 == 26`. Deleting `grpcio==1.80.0` failed exactly the three gRPC closure cases with `backend/tests reaches grpc, which is provided by grpcio, and backend/requirements-dev.txt pins none of them`. Both perturbations were restored byte-for-byte and re-verified | D101, D144, D196, D311, D312, D313 |
