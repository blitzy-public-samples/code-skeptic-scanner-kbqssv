...
| §F | **Direction B** — test artifact → the construct it covers | Every file of the new suite, and the production construct or infrastructure obligation behind it | 96 rows naming 98 of the 102 change-set paths |
...
because the artifacts they named were removed. The 96 rows below therefore name 98 distinct paths, and the
...
| F31 | `e2e/playwright.config.ts` | Infrastructure: spec discovery, the owned `webServer`, artifact retention and both reporters. Also the spec fixtures behind H9 and H13 — the `test`/`expect` every spec imports, the context-wide `noEgress` abort with teardown attribution, and the `unInterceptedApiRequests` ledger that fails a spec which forgot an intercept |
...
| F39 | `.gitignore` | Infrastructure: 18 patterns covering every artifact the four suites produce, plus the two dependency trees and the documented virtual environments - and nothing else. Seven rules are path-anchored (`backend/reports/`, `frontend/coverage/`, `frontend/reports/`, `e2e/node_modules/`, `e2e/test-results/`, `e2e/playwright-report/`, `e2e/reports/`) and reach no further than the directory that produces them; the remaining eleven (`__pycache__/`, `*.py[cod]`, `.pytest_cache/`, `.coverage`, `coverage.xml`, `coverage.json`, `coverage.lcov`, `htmlcov/`, `node_modules/`, `.venv-backend/`, `venv/`) match at any depth by design, which is what covers the coverage files pytest writes into `backend/` |
...
| F83 | `e2e/tests/analytics.spec.ts` | Route `/analytics` over `src/components/Analytics` (`TrendCharts`) — heading and `canvas#trendChart` surviving a **rejected trend request**, and the request issued once per mount for the harness date range. The **chart-construction** ceiling (§G G4) is *not* observed at this layer: reaching the `new Chart(...)` call needs a resolved series, `e2e/harness/stubs/analyticsService.ts` rejects on every path, so `frontend/src/components/Analytics.test.tsx` mocks `getTrendData` and covers the jsdom side. On the one path that opts into `e2e/harness/stubs/analyticsService.ts`'s forwarding header the ceiling itself is measured rather than skipped: an unregistered chart part, the uncaught escape from the passive effect, and the unmount that follows (§G G4). It also records the canvas contributing no node to Chromium's accessibility tree (§G G9) |
...
| F85 | `e2e/tests/harness-fixtures.ts` | Infrastructure: the shared spec fixtures behind H9 and H13 — the context-wide `noEgress` abort with teardown attribution, and the `unInterceptedApiRequests` ledger that fails a spec which forgot an intercept, and `consumeAbortedRequestUrls()`, which lets `isolation.spec.ts` (F93) acknowledge the refusal it deliberately caused, and `browserDiagnostics`, which attaches every console message and page error as evidence and fails a spec at teardown for any `console.error` or `pageerror` it did not declare (H16 and the diagnostics half of H9) |
| F86 | `.github/workflows/ci.yml` | Infrastructure: the test steps and their immediate install prerequisites, plus the new `e2e` job. Every step names a manifest and a working directory that exist: `pip install -r backend/requirements-dev.txt`; `npm install` under `working-directory: frontend`; the gated `pytest` under `working-directory: backend`; `npm run test:ci` under `working-directory: frontend`; and in the `e2e` job, `npm install` then `npm run browsers:verify` then `npm test`, all three under `working-directory: e2e`, so the runner is `e2e/node_modules/.bin/playwright` and the config is the one beside it. No step invokes the browser downloader; the job instead walks four candidate paths on the runner image, fails closed when none is executable, publishes the resolved path as `PLAYWRIGHT_CHROMIUM_EXECUTABLE` through `$GITHUB_ENV` so no step-level override can blank it, and validates it through `e2e/scripts/verify-browser.js` (D236, D237). The Codecov paths are `./backend/coverage.xml` and `./frontend/coverage/coverage-final.json`, with the `backend` and `frontend` flags unchanged. The `flake8`, `mypy` and `npm run lint` steps are deliberately untouched and independently broken (§23 of the log) |
...
| F89 | `blitzy-deck/executive-summary.html` | Rule 4 (Executive Presentation): the leadership summary of this programme, 16 sections over four slide types. Its KPI panel carries measured run figures — `0%` executed coverage before this work, `1,136` tests now passing across the five layers, `93%` backend coverage against the 90% bar, and `42` deliberate defects each caught — and its migration slide carries the `7 / 3 / 15 / 25` disposition sourced from the summary table at the top of this file. Every claim about enforcement is qualified on the face of the deck: the pipeline is described as authored rather than exercised, the 90% gate is paired with the totals that round to `90.00` and still pass, and the empty allowed-origin default, the deferred security patch and the test-only shims appear as named residual risks |
| F90 | `backend/.coveragerc` | Infrastructure: `[report] precision = 2`, which is the value `pytest-cov` reads when it calls `coverage.results.should_fail_under(total, fail_under, precision)`. Without it the default precision of 0 rounds a total to the nearest whole percent before comparing, so `--cov-fail-under=90` exits zero on any total from 89.50 upward while printing `FAIL`. It declares no `fail_under` (that would gate the plan's plain measurement command too) and no `[run] source` (that would override the `--cov=` scoping the gate depends on) |
| F91 | `backend/tests/test_coverage_gate.py` | Infrastructure: the gate contract of F90 — the file exists, the parsed `precision` is 2, `Coverage(config_file=...).config.precision` is the attribute `pytest-cov` actually falls back to, `89.5` / `89.93` / `89.99` fail at precision 2 and pass at precision 0 (the removed behaviour asserted directly), `90.0` / `90.01` / `93.33` / `100.0` pass, the residual `[89.995, 90)` band is admitted **and** prints as `90.00` so status and message can never disagree, and `pytest.ini`'s `addopts` carries no `--cov` |
| F92 | `backend/tests/unit/test_egress_guard.py` | Infrastructure: the ownership contract of the `_EgressGuard` in F6 — an endpoint is keyed on `(family, type, protocol, normalized address, port)` rather than on a bare port number, so a UDP target cannot ride a TCP listener's authorization, a second loopback address cannot ride the first, and an `AF_UNIX` socket is admitted only for a path this process bound; the entry is refcounted and removed on `close`, on `detach` and on garbage collection, so a closed port's number does not stay authorized; and an owned bind to the unspecified address authorizes that family's loopback addresses on that exact port only |
| F93 | `e2e/tests/isolation.spec.ts` | Infrastructure: the property behind H16 — that a spec's own `page.route`, which Playwright matches before the context-wide rule, claims the harness origin and no other. One parameterised case per intercepted request shape (`/undefined/tweets*`, `/tweets*`, `/api/trends*`, `/api/config/twitter`), each proving the harness URL is claimed and the same path on a foreign origin is refused and ledgered instead |
...
| F96 | `e2e/scripts/verify-browser.js` | Infrastructure: the browser-prerequisite gate behind `npm run browsers:verify` and behind the `e2e` CI job's validation step. Checks `PLAYWRIGHT_CHROMIUM_EXECUTABLE` and, only if unset, the exact path `chromium.executablePath()` reports; exits 0 printing the absolute path a run will launch, exits non-zero naming both routes and the exact path each was checked at, and fetches nothing. Covered by H12 (D236) |
| F97 | `e2e/scripts/require-browser.js` | Infrastructure: the browser prerequisite the end-to-end layer cannot obtain for itself. Resolves an executable from `PLAYWRIGHT_CHROMIUM_EXECUTABLE`, then the platform's vendor install locations, then Playwright's cache; validates that it is a launchable file; publishes the resolved path to `$GITHUB_ENV`; and exits non-zero with per-platform provisioning instructions when there is none. It is the fail-closed half of the CVE-2025-59288 remedy, `browsers:verify` being the download-free reporter that always exits 0 (H12, D264) |
| F98 | `docs/testing/dashboard-extract.py` | Rule 2 (Observability): the **producer** behind F88. Reads the JUnit XML and coverage artifacts of all three runners and emits the filled panels as markdown or JSON, so a dashboard reading is derived from artifacts rather than typed by hand; refuses a zero-case result stream instead of reporting a clean run of nothing, and treats a missing artifact as fatal under `--require-all` (D253, D263). Covers no production symbol — its subject is the artifact contract in F1, F42 and F86
...
|---|-----------|---------------------------|---------------------|-----------|
| H1 | Every active line of the backend manifest is an exact pin, and every pinned version is the version actually installed | `backend/requirements-dev.txt` (25 pins, `python-jose[cryptography]` among them at the plan's `3.3.0`) and `backend/tests/test_dependency_closure.py` (an exact-pin grammar applied to **every** active line rather than to the lines that happen to match, an assertion that the parsed-pin count equals the active-line count, a distribution-name uniqueness check, and one parametrised installed-version assertion per pin) | 43 cases pass: 25 pins asserted against `importlib.metadata.version`, all matching, plus the grammar, count, uniqueness and comment cases and 12 parametrised rejection forms. Negatively validated twice. Rewriting one `==` as `>=` failed exactly two cases — the exact-pin case naming the line, and the count case reporting `assert 24 == 26`; the manifest was restored and re-verified. Separately, while `python-jose` was pinned at `3.5.0` against the plan's `3.3.0`, the installed-version case named the distribution, the pin and the installed version, which is how that drift was found; the manifest now holds `3.3.0` and the installed tree matches it | D101, D144, D196 |
...
| H12 | The harness process and the browser both come from pinned local artifacts, no automated path anywhere in the repository can download a browser, a run is refused when no browser is present, and the server under test is always the one this configuration started | `e2e/playwright.config.ts` (`webServer.command` naming `node ./node_modules/vite/bin/vite.js`, `reuseExistingServer: false`, `PLAYWRIGHT_CHROMIUM_EXECUTABLE` read through a helper that treats a blank value as absent - an undefined GitHub Actions variable expands to the empty string - and passed through as `launchOptions.executablePath`), `e2e/package.json` (every script a local binary; **no** `install:browsers` script), `e2e/scripts/verify-browser.js` (`browsers:verify` — a filesystem gate over the named executable and then the exact `chromium.executablePath()`, which fetches nothing and exits non-zero when neither is launchable) and the `e2e` job of `.github/workflows/ci.yml` (resolves a browser already on the runner image into that variable, then runs the gate, then runs `npm test` with `working-directory: e2e` — it invokes no downloader and no root `npx`) | The harness starts as `VITE v4.5.14` on the resolved loopback port with no `npx` involved. The gate was measured in four states: the real executable → exit 0 printing its absolute path; a nonexistent path → exit 1; unset with an empty cache → exit 1 naming the exact expected cache path; a directory → exit 1. The predecessor, `playwright install --dry-run chromium`, exited **0** in all four, including with `fs.existsSync(chromium.executablePath())` false — which is why it was replaced. `npm run test:list` from `e2e/` reports 19 tests in 5 files through the pinned local binary, a repository-wide grep finds no invocation of `playwright install` outside explanatory prose, and no `npx` appears anywhere in `ci.yml` at all. Negatively validated a second way: `npx --no-install playwright --version` at the repository root reports it would have to fetch `playwright@1.62.1`, which is exactly what the previous root-invoked CI step would have run | D111, D112, D113, D129, D236, D237, D242, D264 |
...
- **The change set** — `git diff --name-status --no-renames` against the pre-work baseline reports **102
  operations: 94 additions, 4 modifications, 4 deletions**. Eighty-five of those paths are the frozen plan’s,
  set-equal to what it authorizes; the remaining seventeen are artifacts the remediation checkpoints added, and
  ...
  direction of this document and in no diff against the baseline. That 102 is the denominator every claim below
  is measured against.
...
F9 — name *sections of the same file*, `backend/tests/conftest.py`. Two ids are also absent by decision rather
than by accident: F30, F72 and F75 named `e2e/harness-origin.ts`, the two `.npmrc` files and `frontend/src/test-utils/configured-base.ts`, all four of which were
removed (D275, D276, D285, D288). So the two figures are different by construction and both are stated:

| Figure | Value | How to reproduce it |
|--------|-------|---------------------|
| §F rows | **96** | `F1` … `F98`, with F30, F72 and F75 withdrawn and F77a present, one per artifact or per distinguishable concern within one artifact |
| Change-set entries | **101** | `git diff --name-status -M 252bfff` plus the working tree: 4 modified, 93 added, 1 renamed, 3 deleted |
| Logical operations | **102** | the 101 entries plus one, because the single rename — the extension-less `backend/app/api/routes` file becoming `backend/app/api/routes/tweets.py` — is a delete and a create |
| Distinct change-set paths | **102** | the same set counted as filesystem paths rather than entries: the rename names two paths, so paths and operations coincide here while entries do not |
| Change-set paths named in §F | **98** | the union of the back-quoted paths in every §F artifact cell, `{a,b,c}` braces expanded and backend- or frontend-relative forms resolved, intersected with the 102 |

**102 − 98 = 4, and all four are named.** Three are the deleted legacy modules — `backend/tests/test_api.py`,
...
this checkpoint edited no production module beyond the two authorized touches — so 34 file-like tokens in §F
cells resolve to no change-set path. That is the intended state, not a gap: a covering row that named only
files this change set touched would be a row that covers nothing. The check that does close is the one above,
change set → §F, and §I is what inverts it at the level of subjects rather than paths.
...
is the file each row names, and H12 named the `e2e` package only — while `.github/workflows/ci.yml`
downloaded a browser through the path CVE-2025-59288 concerns and invoked a runner the package does not
own. The mapping was complete and the guarantee was still being broken by a consumer outside its subject.
H16 exists to close that: it names the workflow, and it carries its own verification evidence. A guarantee
row is therefore only as strong as the set of files it names, and adding a consumer means checking whether
an existing row's subject has to widen — or, as here, whether a new row has to name it.

H17 was written by applying that same test a second time rather than by restating it. A stub-refusal
guarantee has three independent consumers — the workflow, the `e2e` package's list script, and the
dashboard extractor — and a row naming only one of them would have been complete as a mapping while two
consumers went on publishing a zero-case stream as a result. So the row names all three, and it records
which of the three runners actually has the defect and which does not.

The one thing no row in this document can evidence: **this workflow has never run on GitHub Actions from
this branch.** H16's evidence is the workflow file plus a local execution of each step's script. That is
