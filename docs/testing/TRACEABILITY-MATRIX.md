...
| F100 | `docs/testing/SECURITY-GAPS.md` | Rule 1 and Rule 3: the current-state security gap register. Every security exposure still present in the delivered tree, each stated as a fact about the code, with the AAP clause that puts it out of the authorized write surface, what it is worth to an attacker, and what a human must do. 28 exposures in five groups — application code, pinned dependencies, infrastructure and deployment, the documentation that overstated the position, and the executive deck as a served artifact, the one group fixed by a user-specified rule rather than an AAP exclusion. The last three ids, 26 to 28, were appended when a runtime QA pass measured the credential screen in a real browser: the four values React mirrors into the `value` attribute and never clears, the uncancellable and unthrottled write, and the absent normalisation and length bound. Ids are append-only, so they sit at the end of group one rather than beside row 9 (D401). Covers no production symbol and asserts nothing: it exists because a decline recorded only in a review reply is indistinguishable, later, from a finding nobody noticed (D324) |
