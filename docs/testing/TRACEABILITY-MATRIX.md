...
| §B | msw handler → caller and route | Every handler exported by `frontend/src/test-utils/handlers.ts` | 9 |
| §C | Backend route → frontend callers | Every route the backend implements or is called at | 5 |
| §D | Divergence → the suite obliged to assert it | Every mismatch §A records, plus the two behaviours the security-gate remediation added | 15 |
...
| §F | Test artifact → the construct it covers | Every file of the new suite, and the production construct or infrastructure obligation behind it | 73 |
| §G | Coverage ceiling → the assertion that stands in for it | Every branch no test can execute without changing production | 8 |
| §H | Harness guarantee → the artifact that enforces it | Every determinism, credential, egress and isolation guarantee the three suites rest on, and the file that implements it | 14 |

Coverage of the mapped set is complete: all four routes registered in `ROUTE_CONTRACTS` and all nine
...
That stand-in matters for one route. `GET /tweets` is the only route whose outcome **depends** on it: with
the real `get_db` the route answers `500`, and the `200`/`422` pair below is what it answers once the
dependency is replaced. §A states both, X13 records why, and B5 and B9 model them separately. For every
other route the stand-in changes nothing.
...
|---|--------|------------------|----------------------------|------------------------------------|------------------------|----------------------|
| A1 | `services/api.ts` `fetchTweets(page, limit)` | `GET undefined/tweets?page=<page>&limit=<limit>` | `GET /tweets`, query `skip: int = 0`, `limit: int = 100`, returns `List[Tweet]` | `500` `Internal Server Error` unoverridden — `db.query(Tweet)` on the Firestore `Client`; `200` with the list under a SQLAlchemy-shaped `get_db` override when both coerce to `int` | isolation `GET */tweets` → 200 with three tweets; `currentBehaviorTweetsHandlers()` → 500; `dependencyOverriddenTweetsHandlers()` → 200 | X1, X2, X13 |
| A2 | `services/twitterService.ts` `getLatestTweets(count)` | `GET undefined/tweets?page=<count>&limit=undefined` — calls the two-parameter `fetchTweets` with one argument | same route | `500` unoverridden; under the override `422` `{"detail":[{"loc":["query","limit"],"msg":"value is not a valid integer","type":"type_error.integer"}]}` | isolation → 200; `currentBehaviorTweetsHandlers()` → 500; `dependencyOverriddenTweetsHandlers()` → 422 with that exact body | X3, X13 |
| A3 | `components/Dashboard` `RealTimeFeed`, mount effect and 30 s interval | `GET undefined/tweets?page=undefined&limit=undefined` — calls `getLatestTweets()` with no argument | same route | `500` unoverridden; under the override `422`, `loc` naming `limit` | isolation → 200; `currentBehaviorTweetsHandlers()` → 500; `dependencyOverriddenTweetsHandlers()` → 422 | X3, X4, X13 |
...
| B5 | `currentBehaviorTweetsHandlers()` | current backend behaviour, dependency **not** overridden | `GET <origin>/*/tweets`, one handler per allowed origin | A1, A2, A3 | `500` `Internal Server Error`, `text/plain; charset=utf-8`, for every request, for the reason X13 records |
...
| B9 | `dependencyOverriddenTweetsHandlers()` | current backend behaviour **under** `app.dependency_overrides[get_db]` | `GET <origin>/*/tweets`, one handler per allowed origin | A1, A2, A3 | `200` when `skip` and `limit` coerce to `int`, otherwise `422` naming the first failing parameter |
...
into one fresh array; it introduces no route of its own and deliberately **excludes B9**, because B5 and B9
model the same route under two different dependency dispositions and only one of them can be installed at a
time. A suite installs B5 to assert what the assembled application returns, and B9 to assert the contract the
backend integration layer asserts under an override. `<origin>` is each entry in
...
isolation violation, not a response; see §H row H7.
...
`dependencyOverriddenTweetsHandlers` (B9, the layer-2 set B5 excludes), `IsolationViolationKind` / `IsolationViolation` / `recordedIsolationViolations` /
...
| X13 | `GET /tweets` cannot answer `200` or `422` as assembled: `get_tweets` calls `db.query(Tweet)` on the Firestore `Client` that `get_db` yields, and that client has no `query`, so the route answers `500` until a test replaces the dependency | `routes/tweets.py` line 12 vs `db/firestore.py` `get_db` | a suite installing B5 asserts the unoverridden `500`; the pagination and coercion contract is asserted only through B9, under `app.dependency_overrides[get_db]` with a SQLAlchemy-shaped fake, and no suite may present that outcome as the assembled application's behaviour |
...
| X15 | a tweet id carrying path syntax silently retargets the request: `fetchTweetById('tweet 1/../7')` interpolates the id verbatim, and the URL parser then removes the preceding segment, so the network receives `/undefined/tweets/7` and the caller is answered with the record for `7` | `services/api.ts` line 14 interpolates the id with neither validation nor `encodeURIComponent` | the API service suite asserts both observation points - the pre-adapter string against a mocked `axios`, and the normalised pathname, the bound route parameter and the returned record over the real adapter; the id is not encoded or repaired on production's behalf (D149) |
...
| F60 | `frontend/src/services/api.test.ts` | `src/services/api.ts` — the literal `undefined/` base URL, the URL each of the three functions constructs, the `response.data.generatedResponse` unwrap, and rejections reaching the caller untransformed |
| F61 | `frontend/src/services/twitterService.test.ts` | `src/services/twitterService.ts` — `getLatestTweets` calling the two-parameter `fetchTweets` with one argument, `getTweetDetails`, and the log-then-rethrow-the-original disposition |
...
| F73 | `frontend/src/test-utils/dependency-closure.test.ts` | Infrastructure: the npm counterpart of `backend/tests/test_dependency_closure.py`. Covers no production symbol - it asserts that `frontend/package.json` and `e2e/package.json` declare exact versions only, that each `frontend` pin is what `frontend/node_modules` holds, and that `.gitignore` carries no active `package-lock.json` rule |
...
| H7 | No frontend request escapes to a real host, including one issued while a test module is still being evaluated | `frontend/src/test-utils/setup-jest.ts` (`server.listen({ onUnhandledRequest })` at setup-module scope, `assertNoIsolationViolations()` then state reset in a global `afterEach`) and `frontend/src/test-utils/handlers.ts` (frozen `ALLOWED_REQUEST_ORIGINS`, no mutator, absolute per-origin patterns, ledger entries for an out-of-scope origin) | 11 Jest assertions in `setup-jest.test.ts`, including one proving an import-time request is intercepted, one proving the allow-list cannot be widened (`push` throws), and one proving a request to a non-loopback origin matches no handler, is never performed, and lands in the ledger as `kind: 'unhandled-request'`. Negatively validated: moving `listen()` back into `beforeAll` made the module-scope probe report `ESCAPED AxiosError: Network Error` | D103, D108, D136 |
| H8 | Every direct dependency of both npm manifests is declared as an exact version, each `frontend` pin is the version the installed graph resolves, and no `.gitignore` rule can suppress the lockfile that would otherwise carry the integrity closure | `frontend/package.json` and `e2e/package.json` (exact pins throughout, 28 and 3 entries), `frontend/src/test-utils/dependency-closure.test.ts` (a case per entry for the pin shape, a case per `frontend` entry comparing it against `node_modules/<name>/package.json`, a non-empty guard per manifest, and a case asserting the absence of a `package-lock.json` ignore rule) and `.gitignore` (that rule removed) | 62 cases pass; `npm ls --depth=0` exits 0 in both packages with no invalid, missing or extraneous entry. Negatively validated: reintroducing `"axios": "^1.19.0"`, mis-declaring `chart.js` as `4.4.0` and re-adding the `package-lock.json` ignore failed exactly four cases, naming the specifier, the two versions and the offending rule. Not covered, deliberately: the transitive graph, for which no integrity-hashed reference exists without a lockfile | D109, D146, D147, D148 |
