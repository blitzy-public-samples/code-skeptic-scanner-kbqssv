...
|---|----------|----------------|----------------------|
| 14 | `backend/requirements-dev.txt` pins versions with published advisories, including the `python-jose` set, `python-multipart`, and Starlette below its fixed releases. `python-jose` already carries an escalated `SECURITY EXCEPTION` block in that file rather than a silent pin. | Depends on the advisory; the JWT ones matter only once row 1 or row 3 makes tokens reachable. | Raise the Python floor above 3.9, then upgrade. Doing so requires re-pinning `pydantic`, `tweepy`, `openai` and `httpx` together and reworking the code that depends on each — a migration, not a bump. |
