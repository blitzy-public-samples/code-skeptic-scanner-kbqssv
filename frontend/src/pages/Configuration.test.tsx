/**
 * Suite over `frontend/src/pages/Configuration.tsx`, the configuration page. Every test below is skipped, and
 * the skip titles name the symbol that stops the page rendering.
 *
...
describe.skip(
  'pages/Configuration - SKIPPED: useAppDispatch is not a function (not exported by src/store/index.ts)',
  () => {
    beforeEach(() => {
    ...
    describe('the mount read', () => {
      it('shows the loading view, and nothing of the page body, while the read is in flight', async () => {
      ...
      it('calls the configuration read exactly once, with no arguments', async () => {
      ...
      it('replaces the loading view with the page container and its heading once the read resolves', async () => {
    ...
    describe('the document the read produced', () => {
      it('dispatches the fetched document into the config slice, which records it and the updated status', async () => {
        const { store } = renderWithProviders(<Configuration />);
      ...
      it('hands all three child sections undefined: the selected object is the whole slice state', async () => {
        const { store } = renderWithProviders(<Configuration />);
    ...
    describe('saving a section', () => {
      it('sends the service one document: the selected object merged with the edited section', async () => {
      ...
      it('dispatches that same document into the config slice and leaves the page mounted', async () => {
        const user = userEvent.setup();
    ...
    describe('failure disposition', () => {
      it('renders the fixed fetch-failure message and dispatches nothing when the read rejects', async () => {
      ...
      it('renders the fixed save-failure message and replaces the page when the write rejects', async () => {
    ...
    describe('the child element types the page composes', () => {
      it('finds none of the three names on @/components/Configuration, which exports only a default', () => {
